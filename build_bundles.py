...
def write_stats(stats):
    """Page counts, for the /bundles pages to quote.

    Only this script knows how many pages a merged PDF ends up with, and the
    catalogue is built before the bundles are, so it cannot carry the number.
    Writing it here keeps the site's "N guides · N pages" line derived rather
    than typed.
    """
    rows = ',\n'.join(
        f"  {{ slug: '{slug}', pages: {pages}, sizeKb: {size_kb:.0f} }}"
        for slug, (pages, size_kb) in sorted(stats.items())
    )
    out = os.path.join(BASE, 'lib', 'bundle-stats.generated.ts')
    ...
    print(f'wrote lib/bundle-stats.generated.ts — {len(stats)} bundles')
...
def main():
    ...
    for bundle in BUNDLES:
        ...
        stats[bundle['slug']] = (len(writer.pages), size_kb)
