...
def build_closing(meta, S):
    flow = []
    for entry in meta.get('read_next', []):
        flow.append(Paragraph(f"<b>{entry['title']}</b>", S['ng_title']))
        flow.append(Paragraph(entry['desc'], S['ng_desc']))
        url = entry['url']
        flow.append(Paragraph(f'<link href="https://{url}" color="#f97316">{url}</link>', S['ng_link']))
...
# ── entry point ──────────────────────────────────────────────────────────────
def render(md_path, out_dir=DEFAULT_OUT):
    ...
    story = build_cover(meta, S) + parse_body(body, S) + build_closing(meta, S)
