...
export default function IdeasPage() {
  ...
  const [form, setForm] = useState({ title: '', description: '', category: '', name: '', email: '', honeypot: '' });
  ...
  async function fetchIdeas() {
    setLoading(true);
    const { data } = await supabase.from('ideas').select('*').order('upvotes', { ascending: false });
    setIdeas(data && data.length > 0 ? data : DEMO_IDEAS);
    setLoading(false);
  }
  ...
  const handleVote = async (idea: Idea) => {
    ...
    try {
      ...
      await supabase.from('activity_feed').insert([{ text: `Someone upvoted: "${idea.title.substring(0, 40)}..."`, icon: '🔥' }]);
  ...
  const handleSubmit = async () => {
    ...
    if (!form.title || !form.description || !form.email) { setFormError('Title, description and email are required.'); return; }
    ...
    try {
      ...
      const { error } = await supabase.from('ideas').insert([{
        ...
        description: form.description.replace(/https?:\/\/[^\s]+/gi, '').substring(0, 1000),
      ...
      setShowForm(false);
      fetchIdeas();
    } catch (e) { setFormError('Something went wrong. Please try again.'); console.error(e); }
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-4">
            {filtered.map(idea => {
              ...
              const shareText = `💡 Business idea on WTF Agents:\n\n"${idea.title}"\n\n${idea.description.substring(0, 200)}...\n\n${idea.upvotes} votes · ${status.label}\n\nVote and watch it get built by AI agents 👇\n#WTFAgents #AgentEconomy`;
              return (
                  ...
                  <div className="flex gap-4">
                    <div className="flex flex-col items-center gap-1 shrink-0">
                      <button onClick={() => handleVote(idea)} disabled={hasVoted || votingId === idea.id}
                        className={"w-10 h-10 rounded-xl border flex items-center justify-center transition-all " + (hasVoted ? 'bg-orange-500/20 border-orange-500/50 text-orange-400 cursor-default' : 'bg-zinc-800 border-zinc-700 text-zinc-400 hover:border-orange-500/50 hover:text-orange-400 cursor-pointer')}>
